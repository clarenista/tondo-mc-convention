...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>PSP Convention Meeting</title>
    ...
    <script>
        ...
        ZoomMtg.init({
            debug:false,
            leaveUrl: '{{ config('app.url') }}',
            showMeetingHeader : false,
            disableInvite : false,
        });
        $.get("/api/v1/guests/zoom/join/config?api_token=" + localStorage.getItem('access_token')).done(function(data){
            ZoomMtg.join(data);
        });
    </script>
</html>
