    ...
    <div class="row">
        <div class="col-md-12">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ url('/cms/banners') }}">Banner</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Add Banner</li>
                </ol>
            </nav>
        </div>
        <div class="col-md-12">
            <a href="{{ url('/cms/banners') }}" class="btn btn-info float-right">RETURN</a>
        </div>
        <div class="col-md-6">
            <form action="{{ route('cms.banners.store') . ( $asset->id ? '/' . $asset->id : '') }}" method="post">
                @csrf
                @isset($asset->id)
                {{ method_field('PUT') }}
                @endisset
                ...
                <div class="form-group">
                    <button class="btn btn-success btn-block">ADD NEW BANNER</button>
                </div>
            </form>
        </div>
    </div>
